...
# Tenta importar DB_TYPE com fallback
try:
    from database import DB_TYPE
except Exception as e:
    print(f"Erro ao importar DB_TYPE: {e}")
...
def atualizar_progresso(pilar, novo_progresso, user_email):
    if not is_admin_user:  # apenas admin
        return False
    try:
        conn = get_connection()
        cursor = conn.cursor()
...
def remover_entrega(entrega_id):
    ...
    try:
        ...
        print(f"Erro ao remover entrega: {e}")
        return False
...
# Inicializar tabelas (com try/except para não quebrar)
try:
